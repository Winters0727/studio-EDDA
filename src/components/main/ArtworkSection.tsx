...
import { ArtworkArticle, ArtworkContent, ArtworkBackground, ArtworkWrapper } from "@styles/main/artwork-section.style";
...
const ArtworkSection: FC = () => {
    ...
    return (
        <ArtworkWrapper ref={(el) => (targetRefs.current[0] = el as HTMLDivElement)} id="artwork"  className={entries["artwork"] ? "scrolled" : ""}>
            <SectionTitle
                text={{
                    title: "Art Work",
                    subTitle: "Art",
            ...
            <ArtworkContent className={entries["artwork"] ? "scrolled" : ""}>
                <ArtworkArticle className={entries["artwork"] ? "scrolled" : ""}>
                    ...
                    />
                    <ArtworkFaceButtons paths={char.imagePaths.faces} color={char.color.main} />
                </ArtworkArticle>
                <ArtworkCharacterImage name={char.name} path={char.imagePaths.standing} color={char.color.main} className={entries["artwork"] ? "scrolled" : ""} />
            </ArtworkContent>
